...
describe("canonical text templates", () => {
  it("normalizes the legacy layers payload into a canonical scene graph", () => {
    const artifact = normalizeTextTemplateArtifact({
      ...
      canvasWidth: 1920,
      canvasHeight: 1080,
      layers: [{ id: "title", kind: "text", role: "primary", x: 100, y: 200, width: 800, height: 120, content: "Name" }],
    });
  ...
  it("applies typed control values through the compiler", () => {
    const artifact = normalizeTextTemplateArtifact({
      id: "title", label: "Title", category: "title-card", duration: 2,
      nodes: [{ id: "title", name: "Title", type: "text", x: 0, y: 0, width: 500, height: 100, text: "Default", style: { fontSize: 48 } }],
      controls: [{ id: "headline", label: "Headline", type: "text", defaultValue: "Default", target: { nodeId: "title", propertyPath: "text" } }],
    });
    const compiled = compileTextTemplate(artifact, { target: "editor", time: 0, controlValues: { headline: "Updated" } });
    expect(compiled.layers.find((layer) => layer.id === "title")?.text).toBe("Updated");
  });

  it("rejects controls that target unknown nodes", () => {
    const artifact = normalizeTextTemplateArtifact({
      id: "broken", label: "Broken", category: "title-card", duration: 2, nodes: [],
      controls: [{ id: "headline", label: "Headline", type: "text", defaultValue: "x", target: { nodeId: "missing", propertyPath: "text" } }],
    });
