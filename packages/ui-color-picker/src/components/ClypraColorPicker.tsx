...
const ClypraColorPickerInner = forwardRef<ClypraColorPickerHandle, ClypraColorPickerProps>(
  (
    ...
    const resolvedPlacement = (placement || position || 'bottom-start') as ColorPickerPlacement;

    const middleware = useMemo(() => {
      const list = [];
      if (offset !== undefined && offset !== null) {
        list.push(floatingOffset(typeof offset === 'number' ? offset : offset));
      }
      if (flip) {
        list.push(floatingFlip(typeof flip === 'object' ? flip : undefined));
      }
      if (shift) {
        list.push(floatingShift(typeof shift === 'object' ? shift : { padding: 8 }));
      }
      return list;
    }, [offset, flip, shift]);
    ...
    const [side, align = 'center'] = computedPlacement.split('-');
