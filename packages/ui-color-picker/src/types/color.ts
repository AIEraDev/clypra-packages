...
export type ColorPickerStrategy = 'fixed' | 'absolute';

export type ColorPickerOffset =
  | number
  | {
      mainAxis?: number;
      crossAxis?: number;
...
export interface ClypraColorPickerProps {
  ...
  /**
   * Floating UI Placement for the popover dialog.
   * Supports all 12 positions: 'top', 'top-start', 'top-end',
  ...
  /**
   * Alias for `placement`. Supports all 12 positions like Floating UI.
   */
  position?: ColorPickerPlacement;

  /**
   * Distance from reference trigger in pixels, or an offset configuration object.
   * Defaults to 6.
   */
  offset?: ColorPickerOffset;

  /**
   * Whether to flip the popover when it overflows the viewport boundary.
   * Defaults to true.
   */
  flip?: boolean | { fallbackPlacements?: ColorPickerPlacement[]; padding?: number };

  /**
   * Whether to shift the popover along the boundary to prevent clipping.
   * Defaults to true.
   */
  shift?: boolean | { padding?: number };

  /**
   * Floating strategy: 'fixed' or 'absolute'.
   * Defaults to 'fixed'.
   */
  strategy?: ColorPickerStrategy;

  /**
   * Whether to automatically update position on scroll, resize, or layout changes.
   * Defaults to true.
   */
  autoUpdate?: boolean;
}
